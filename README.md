...
Todo el trabajo ocurre dentro del navegador. Importas los archivos, editas y exportas sin que ningún fotograma salga de tu equipo, sin instalar nada y sin crear una cuenta. Es gratuito y de código abierto, con todas las funciones disponibles.

Está pensado para quien tiene un video concreto entre manos y quiere resolverlo el mismo día: un recorte con música, una pieza vertical para redes, un tutorial grabado de la pantalla o una entrevista en la que hay que tapar una cara o una matrícula. Se ocupa del trabajo cotidiano de forma directa, no de reemplazar a una suite de escritorio.
...
- **Carriles de texto, imágenes y audio.** Además del video, hay un carril para texto y figuras, otro para imágenes y otro para audio, cada uno con varias filas para separar los elementos que coinciden en el tiempo, hasta seis. Un elemento cambia de fila arrastrándolo, y una fila nueva nace con el botón del carril o soltando un bloque por debajo de la última. Cada carril ajusta el alto de sus filas tirando de su borde y guarda un nombre que se edita con un clic.
...
- **Imágenes.** Logos y fotos superpuestos, en su propio carril de la línea de tiempo, con tamaño del 3 al 200 % del ancho del lienzo, opacidad, corrección de color propia y recorte con la misma herramienta de recuadro que el video. Al redimensionarlas mantienen su proporción, y con Shift pulsado se deforman libremente.
