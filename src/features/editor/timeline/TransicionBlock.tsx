...
export default function TransicionBlock({
  ...
  function estirar(e: ReactPointerEvent) {
    e.stopPropagation()
    e.preventDefault()
    // al agarrar el tirador se selecciona el clip y se DESPLIEGA el panel de transiciones (la categoría
    // de la derecha), igual que al pulsar un impacto sale su editor. además se le dice al panel qué lado
    // se tocó (inicio o final) para que resalte esa misma. todo esto vale también con shift
    seleccionar(clip.id)
    setHerramienta('transiciones')
    setCategoriaClip('transiciones')
